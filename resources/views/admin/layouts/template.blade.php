                ...
                <ul class="navbar-nav">
                    <li class="nav-item font-weight-semibold d-none d-lg-block ms-0">
                        <h1 class="welcome-text">Xin chào, <span
                                class="text-black fw-bold">{{ ucfirst(Auth::guard('admin')->user()->name) }}</span>
                        </h1>
                    </li>
                </ul>
                        ...
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="UserDropdown">
                            <div class="dropdown-header text-center">
                                ...
                                <p class="mb-1 mt-3 font-weight-semibold">
                                    {{ ucfirst(Auth::guard('admin')->user()->name) }}</p>
                                <p class="fw-light text-muted mb-0">{{ Auth::guard('admin')->user()->email }}</p>
                            </div>

                            <a class="dropdown-item" href="{{ route('auth.handle.logout') }}"><i
                ...
                <ul class="nav">
                    @foreach ($menus as $menu)
                        ...
                        @else
                                ...
                                @if (Session::get('level') == 0)
                                    <li class="nav-item">
                                        <a class="nav-link" data-bs-toggle="collapse"
                                            href="#{{ $menu['tag'] }}" aria-expanded="false"
                                            aria-controls="{{ $menu['tag'] }}">
                                            <i class="menu-icon mdi mdi-{{ $menu['icon'] }}"></i>
                                            <span class="menu-title">{{ $menu['label'] }}</span>
                                        ...
                                        <div class="collapse" id="{{ $menu['tag'] }}">
                                            <ul class="nav flex-column sub-menu">
                                                @foreach ($menu['items'] as $item)
                                                    <li class="nav-item">
                                                        <a class="nav-link"
                                                            href="{{ route($item['route'], ['model' => $item['model']]) }}">{{ $item['label'] }}</a>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </div>
                            ...
                            @else
                                <li class="nav-item">
                                    <a class="nav-link" data-bs-toggle="collapse" href="#{{ $menu['tag'] }}"
                                        aria-expanded="false" aria-controls="{{ $menu['tag'] }}">
                                        <i class="menu-icon mdi mdi-{{ $menu['icon'] }}"></i>
                                        <span class="menu-title">{{ $menu['label'] }}</span>
                                        <i class="menu-arrow"></i>
                                    </a>
